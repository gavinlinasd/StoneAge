...
#ifdef _PROFESSION_SKILL			// WON ADD 人物职业技能
	|| CHAR_getWorkInt( GuardianIndex, CHAR_WORKDIZZY ) > 0 	// 堽傳				
	|| CHAR_getWorkInt( GuardianIndex, CHAR_WORKDRAGNET ) > 0 	// 天罗地网
	|| CHAR_getWorkInt( GuardianIndex, CHAR_WORKINSTIGATE ) > 0 // 薬王	
#endif
#ifdef _PROFESSION_ADDSKILL
...
#ifdef _PROFESSION_SKILL			// WON ADD 人物职业技能
					|| CHAR_getWorkInt( attackindex, CHAR_WORKINSTIGATE) > 0	// 薬王	
#endif
#ifdef _PROFESSION_ADDSKILL
...
#ifdef _SKILL_SACRIFICE//照址 vincent add 2002/05/30
...
#ifdef _SKILL_TOOTH
	case BATTLE_COM_S_TOOTHCRUSHE:	//  掛宝 tooth
		flg |= BCF_TOOTH;
		...
		case BATTLE_COM_S_CHAIN_ATK_2:	// 双重攻击
			{
				...
				int evanish_num = CHAR_getWorkInt( defindex, CHAR_WORKDAMAGEVANISH );	// 便
...
int battle_profession_status_chang_fun(int battleindex, int attackNo, int defNo, int charaindex)
{
	...
	switch(skill_type){	
		...
		case BATTLE_COM_S_INSTIGATE:		// 薬王
		...
		case BATTLE_COM_S_THUNDER_ENCLOSE:	// 雷附体
			{
				...
				// 回合数	
				if( (skill_type == BATTLE_COM_S_INSTIGATE) && (skill_level == 10) ){		// 薬王
					turn = 4;
