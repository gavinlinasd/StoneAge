...
int PET_DEBUG_initPetOne( int charaindex)
{
    ...
    ch.data[CHAR_HP] = 100;
    /*    鞳 */
    strcpysafe( ch.string[CHAR_NAME].string, 32, "唾昜ㄠ" );
...
static int _PET_dropPet( int charaindex, int havepetindex, int tofl, int tox, int toy)
{
	...
	if( CHAR_getWorkInt( charaindex, CHAR_WORKTRADEMODE) == CHAR_TRADE_TRADING ){
		CHAR_talkToCli( charaindex, -1, "蝠眢袨怓笢拸楊隍堤唾昜﹝", CHAR_COLORYELLOW );
		return FALSE;
	}
#endif
    // CoolFish: Family 2001/6/13
    if (CHAR_getInt(petindex, CHAR_PETFAMILY) == 1){
    	CHAR_talkToCli(charaindex, -1, "模逜忐誘忤拸楊隍堤ㄐ", CHAR_COLORYELLOW);
    	return	FALSE;
    }
    if (CHAR_getInt(charaindex, CHAR_RIDEPET) == havepetindex ){
    	CHAR_talkToCli(charaindex, -1, "る傚笢腔唾昜拸楊隍堤ㄐ", CHAR_COLORYELLOW);
    	return	FALSE;
    }    
	...
	for( i = x-CHAR_DEFAULTSEESIZ/2 ; i <= x+CHAR_DEFAULTSEESIZ/2 ; i++ ){
		...
		if( count_item > 80 || count_chara > 80 ) {
			CHAR_talkToCli( charaindex, -1, "涴爵眒冪怮茧撥賸ㄛ祥夔婬隍賸﹝", CHAR_COLORYELLOW );
			return FALSE;
		}
	}
 ...
 *     裺  妐蚗哱禱  ��
 * 礎倳
 *  itemindex       int         囮騷  堜騷璃溢蚗袲筒
 *  floor           int         啞痲囮ID
 *  x               int         x淢  
 *  y               int         y淢  
 *  net             BOOL        汜蚗哱極↓袲摯喫敁禱埰堎凝汔竣凝
 * 蒍堇偯
 *  埬      objindex
 *  謄      -1
 ------------------------------------------------------------*/
...
int PET_dropPetFollow( int charaindex, int havepetindex, int tofl, int tox, int toy)
{
    ...
    if (CHAR_getInt(charaindex, CHAR_RIDEPET) == havepetindex){
    	CHAR_talkToCli(charaindex, -1, "る傚笢腔唾昜拸楊躲呴ㄐ", CHAR_COLORYELLOW);
    	return	FALSE;
    }
	if( tofl == -1 ) {
	    for( i  = 0 ; i < 7 ; i  ++ ){
...
void PET_showEditBaseMsg( int charaindex, int toindex, int itemindex, int *work)
{
	int i, maxnums = 6000;
	char buf1[256];
	char buf2[][32]={"勂薯傖酗薹","騵壅薯傖酗薹","厒僅傖酗薹","极薯傖酗薹","夔薯"};
	char buf3[][32]={"湮盟枑詢","謹峈枑詢","謹峈熬屾"};
	...
	for( i=0; i<4; i++)	{
		...
		strcpy( buf1,"\0");
		if( work[i] > maxnums )	{
			sprintf(buf1,"%s 眒冪湛善郔詢賸﹝", buf2[i]);
			work[i] = maxnums;
		}else if( work[i] < 0 )	{
			sprintf(buf1,"%s 眒冪峈錨賸﹝", buf2[i]);
			work[i] = 0;
		}else	{
			if( type > 0 )	{
				if( type > 2 )	{
