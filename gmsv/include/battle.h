...
typedef enum{
  ...
  BATTLE_COM_S_ATTACK_MAGIC,		// 宠物魔法
    ...
    BATTLE_COM_S_SACRIFICE,			//照址		vincent add 2002/05/30
#endif
#ifdef _SKILL_WEAKEN  
...
#ifdef _PET_SKILL_SARS				// WON ADD 蕎彪茸决
	BATTLE_COM_S_SARS,
#endif
...
{
	BOOL	use;	/* 银匀化中月井升丹井 */
	int		battleindex; /* 战斗编号 */
	int		mode;	/* 鰒時式浄  胴＆囁 */
	int		type;	/* 战斗类型 (0:通常)(1:DUEL)(2:BOSS战) */
	int		dpbattle;	/* DP田玄伙井＂ */
	int		norisk;	/* 是否是死亡无风险的战斗? */
	int		turn;	/* 正□件醒 */
...
#define TARGET_SIDE_0	20	// 右侧  指定阵营０
#define TARGET_SIDE_1	21	// 左侧  指定阵营１
...
extern int	gItemCrushRate;		//   破坏  默认
extern BATTLE *BattleArray; 	/* 战斗任务     */
extern int BATTLE_battlenum;	/*     及醒 */
extern char szAllBattleString[BATTLE_STRING_MAX];	/* 战斗使用的命令字符串   */
extern char *pszBattleTop, *pszBattleLast;	/* 战斗使用的字符串的位置   */
extern char szBadStatusString[];	// 状态异常用字符串  
extern int gWeponType;	// 蜇箕及  湛及潘  
extern float gDamageDiv;	// 伤害分割
...
#define CH_FIX_PLAYERLEVELUP	(+2)	// 玩家升级
#define CH_FIX_PLAYERDEAD		(-2)	// 玩家通常死亡
#define CH_FIX_PLAYEULTIMATE	(-4)	// 玩家终极死亡
#define CH_FIX_PETESCAPE		(-1)	// 文喟傲札  突俔

#define AI_FIX_PETLEVELUP		(+5*100)	// 宠物升级
...
#define AI_FIX_SEKKAN 			(-2*100)	// 攻击自己的宠物
#define AI_FIX_PLAYERULTIMATE	(-10*100)	// 自己的主人终极死亡
#define AI_FIX_PETULTIMATE		(-10*100)	// 宠物终极死亡
#define AI_FIX_PLAYERDEAD		(-1*100)	// 自己的主人昏迷
#define AI_FIX_PETDEAD			(-5*100)	// 宠物昏迷
