...
BOOL NPC_QuizInit( int meindex )
{
	...
	if( CHAR_getWorkInt( meindex, CHAR_WORK_QUIZNUM) > ( tbl[0] - 1)){
		print(
		 "%s�F睫磁沭璃,埴�妁uiz腔恀枙杅(%d�  掀堤枙杅(%d枙)祥逋",
	...
	switch( num) {
	  case 0:
		...
		if(NPC_Util_GetStrFromStrWithDelim( argstr,"StartMsg",
			...
			print("%s腔StartMsg祥湔婓", CHAR_getChar(meindex,CHAR_NAME));
			return;
		}
		...
		if(entryflg == FALSE){
			...
			if(NPC_Util_GetStrFromStrWithDelim( argstr,"EntryItem",
				...
				if(NPC_EntryItemDel( talker, buf2)	==FALSE){
					print("耋撿羶衄秏囮(%s)",CHAR_getChar(meindex,CHAR_NAME));
					return;
				}
			}
			if(NPC_Util_GetStrFromStrWithDelim( argstr,"EntryStone",
				...
				if(NPC_EntryStoneDel( talker, buf3)	==FALSE){
					print("踢ヴ羶衄秏囮(%s)",CHAR_getChar(meindex,CHAR_NAME));
					return;
				}
			}
	  ...
	  case 3:
		...
		if(NPC_Util_GetStrFromStrWithDelim( argstr,"ItemFullMsg",
									token, sizeof( token)) == NULL)
		{
			print("%s腔ItemFullMsg祥湔婓", CHAR_getChar(meindex,CHAR_NAME));
			return;
		}
...
//				print("\n %s %d羲溫",CHAR_getChar(talker,CHAR_NAME),(int)PLAYER.ptr);
			...
			/*--喫摯      勗优擭踏躂趙笢堎邳贀摯  坶  蠕禱  拻③埰--*/
			tbl =  NPC_GetQuestion( meindex);
			
			
			/*--  坶倳摯鏍攝蚗袲 tbl[0]勗毀邳贀禱  倜媃倜  坶倳誑毀笢埱趙笢堎 --*/
			if( CHAR_getWorkInt( meindex, CHAR_WORK_QUIZNUM) > ( tbl[0] - 1))
			{
				/*--  坶誑鵿堇埵笢--*/
				/*--隍さ惉    楣  --*/
...
BOOL NPC_QuizAddItem(int talker,char *buf)
{
	...
	if( !CHAR_CHECKITEMINDEX( talker, ret) ){
		...
		CHAR_talkToCli( talker, -1, "耋撿戲眒雛", CHAR_COLORWHITE);
...
BOOL NPC_PlayerCheck(int meindex,int talker)
	...
	if((ptr =(int *) calloc( 1,sizeof(struct pl)))  == NULL){
        print("暮砪极拸楊�楛�%s",CHAR_getChar(meindex,CHAR_NAME));
    	return FALSE;
	}

    /*-- 睽騷璃淏摯鳹鐎禱斐窇趙堁��??-*/
    Player.ptr = ptr;
    Player.talkerindex = talker;	//npc摯騷璃溢蚗袲筒???
	Player.quizno = 0;		//藯凜摯  坶倳
	Player.answer = 0;		//諜搣倳
	Player.ansno = 0;		//紮窇摯  蠕禱斐窇趙堁��  紮窇禱泬溘泬溘勗③媃倜傷勗窅暵)
