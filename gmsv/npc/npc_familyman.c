...

{
	...
	if( buttonproc[button].use == TRUE ) {
		...
		if( newwin == 5 )	{
			...
			{
				//CHAR_talkToCli( talkerindex, -1, "訧跡祥睫ㄐ眒冪樓�趧眢憛�", CHAR_COLORWHITE );
				lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
					WINDOW_BUTTONTYPE_OK, -1, -1,
					makeEscapeString( "\n竭惕К鉊ㄐ斕眒冪樓�趧眢斲芄�", buf, sizeof(buf)));
				return;
			}		
			if( (CHAR_getInt( talkerindex, CHAR_TRANSMIGRATION ) == 0)
				&& (CHAR_getInt( talkerindex, CHAR_LV) < 30)  )
			{
				//CHAR_talkToCli( talkerindex, -1, "竭惕Кㄐ脹撰祥逋﹝", CHAR_COLORWHITE );
				lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
					WINDOW_BUTTONTYPE_OK, -1, -1,
					makeEscapeString( "\n竭惕К鉊ㄐ斕腔脹撰祥逋ㄐ", buf, sizeof(buf)));
				return;
                        }
				...
				lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
					WINDOW_BUTTONTYPE_OK, -1, -1,
					makeEscapeString( "\n竭惕К鉊ㄐ斕斛剕珂俇傖傖�侚騍瓚苺�", buf, sizeof(buf)));
				return;
                        }
			...
			if( CHAR_getInt( talkerindex, CHAR_GOLD ) < 10000 )
			{
				lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
					WINDOW_BUTTONTYPE_OK, -1, -1,
					makeEscapeString( "\n竭惕К鉊ㄐ斕腔扠③忒哿煤祥逋ㄐ", buf, sizeof(buf)));
				return;
			}
		...
		if( newwin == 7 )
		{
			if( CHAR_getInt( talkerindex, CHAR_FMINDEX ) == -1 ) {
				// CHAR_talkToCli( talkerindex, -1, "斕遜帤樓�躽庥弮眢槤翩�", CHAR_COLORWHITE );
				lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
					WINDOW_BUTTONTYPE_OK, -1, -1,
					makeEscapeString( "\n竭惕К鉊ㄐ斕遜羶衄樓�躽庥弮眢槤膛�", buf, sizeof(buf)));
				return;
			}
						
			//if (CHAR_getInt(talkerindex, CHAR_FMLEADERFLAG) == FMMEMBER_LEADER) {
			//	CHAR_talkToCli( talkerindex, -1, "逜酗....", CHAR_COLORWHITE );
			//	return;
			//}
